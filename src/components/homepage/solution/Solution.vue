...
const categories = ref({
  "Creative Production": [
    ...
    {
      id: 3,
      thumbnail: "https://source.unsplash.com/random?decoration",
      title: "Wedding Decoration",
      description: "Layanan yang kami :",
      services: [
        "Indoor",
        "Outdoor",
        "Minimalist",
...
<template>
  <div id="solution" class="md:w-[72rem] mx-auto w-full my-24">
    <div class="max-w-2xl mx-auto text-center">
      <h2
        class="text-2xl font-bold tracking-tight text-gray-900 md:text-3xl sm:text-4xl"
      >
    ...
    <div class="w-full max-w-5xl px-2 py-8 mx-auto sm:px-0">
      <TabGroup>
        <TabList
          class="flex max-w-lg gap-3 p-1 mx-auto space-x-1 rounded-xl bg-amber-200/20"
        >
          <Tab
            ...
            <button
              :class="[
                ...
                selected
                  ...
                  : 'text-slate-500/50 hover:bg-white/[0.12] hover:text-amber-500',
              ]"
            >
              {{ category }}
            </button>
          ...
          >
            <ul class="grid gap-5 mt-5 md:grid-cols-3">
              <li v-for="post in posts" :key="post.id" class="relative h-96">
                ...
                >
                  <ul
                    class="absolute inset-x-0 bottom-0 flex flex-col gap-2 px-5 text-xs font-normal leading-4 rounded-md pb-7 bg-gradient-to-t from-slate-900 text-slate-200"
                  >
                    ...
                    <li>
                      <ul class="flex flex-col gap-2 mt-2 text-slate-500">
                        <li
                          v-for="service in post.services"
                          :key="service"
                    ...
                    <li class="mt-4">
                      <a
                        href="/"
                        class="px-5 py-2 text-sm font-normal transition duration-200 ease-in-out delay-75 border rounded-sm hover:scale-110 hover:translate-y-1 text-amber-300 border-amber-300 hover:border-none hover:bg-slate-50 hover:text-amber-600"
                        >Lihat Layanan</a
                      >
                    </li>
                  </ul>
